...
from config import (
    ...
    MIN_CHUNK_CHARS,
    # Step 4
    # CHROMA_PERSIST_DIR, (不再需要)
    # CHROMA_COLLECTION_NAME, (不再需要)
    EMBEDDING_MODEL,
    RETRIEVAL_TOP_K,
    # Step 5
    CHAT_MODEL,
...
def detect_repeated_headers_footers(pages_text, top_n_lines=2, bottom_n_lines=2, min_repeat_ratio=0.6):
    ...
    if total_pages == 0: return set(), set()
    h_cnt, f_cnt = Counter(), Counter()
    for _, text in pages_text:
        lines = [ln.strip() for ln in (text or "").split("\n") if ln.strip()]
        if not lines: continue
        for ln in lines[:top_n_lines]: h_cnt[_normalize_line(ln)] += 1
        for ln in lines[-bottom_n_lines:]: f_cnt[_normalize_line(ln)] += 1
...
def build_chunks_from_pdf(pdf_path: str) -> List[Dict]:
    ...
    return [{"doc_name": doc_name, "chunk_id": f"{doc_name}::chunk_{idx:04d}", "text": c} for idx, c in
            enumerate(chunks)]


def step3_ingest_to_json(input_dir=INPUT_PDF_DIR, output_path=OUTPUT_CHUNKS_PATH):
    if not os.path.exists(input_dir): raise FileNotFoundError(f"Dir not found: {input_dir}")
    pdfs = sorted([os.path.join(input_dir, f) for f in os.listdir(input_dir) if f.lower().endswith(".pdf")])
    all_chunks = []
    for p in pdfs: all_chunks.extend(build_chunks_from_pdf(p))
...
# =========================
# Step 4: Embeddings & Manual Vector Search (替换 ChromaDB)
# =========================
...
def step4_index_to_chroma(api_key: Optional[str] = None):
    ...
    print(f"Indexing {len(docs)} chunks locally (using Manual Vector Store)...")
...
def step4_query(query: str, top_k: int = RETRIEVAL_TOP_K, api_key: Optional[str] = None) -> Dict:
    ...
    if not query.strip(): raise ValueError("Query is empty")
    ...
    # 计算所有 chunk 的余弦距离 (1 - cosine similarity)
    # 使用 1 - (1 - cosine) 即为 cosine distance
    dists = [float(cosine(q_emb, emb)) for emb in store["embeddings"]]

    # 获取距离最小（相似度最高）的前 k 个索引
...
def load_declarations_facts(parsed_json_path: str):
    if not os.path.exists(parsed_json_path): return {}, []
    with open(parsed_json_path, "r", encoding="utf-8") as f:
        data = json.load(f)
    facts, evidence = {}, []
    for ch in data.get("chunks", []):
        doc, text = (ch.get("doc_name") or "").lower(), ch.get("text", "")
        if "declarations" not in doc and "decp" not in doc: continue
...
def _get_declarations_chunks_from_step3(parsed_json_path=OUTPUT_CHUNKS_PATH):
    if not os.path.exists(parsed_json_path): return []
    with open(parsed_json_path, "r", encoding="utf-8") as f: payload = json.load(f)
    return [ch for ch in payload.get("chunks", []) if
            "declarations" in (ch.get("doc_name") or "").lower() or "decp" in (ch.get("doc_name") or "").lower()]


def _build_declarations_block(intent: str) -> str:
    if intent != "Scenario": return ""
    try:
        facts, evidence = load_declarations_facts(OUTPUT_CHUNKS_PATH)
        if not facts and not evidence: return ""
...
def classify_intent(question: str, api_key: Optional[str] = None) -> str:
    client = _openai_client(api_key)
    system_msg = "You are an intent classifier. Classify: Informational | Clarification | Scenario. Output ONLY the label."
    msgs = [{"role": "system", "content": system_msg}]
    examples = [("What is the deductible?", "Informational"), ("I went to the ER. Covered?", "Scenario")]
    for q, a in examples:
        msgs.extend([{"role": "user", "content": q}, {"role": "assistant", "content": a}])
    msgs.append({"role": "user", "content": question})
    resp = client.chat.completions.create(model=INTENT_MODEL, messages=msgs, temperature=0)
...
def build_answer_instruction(intent: str) -> str:
    # Only change: restructure output sections per request.
    # - "Current answer" -> "Answer" (top)
    # - "Questions" -> "Recommended Questions" (bottom)
    # - Keep Informational simple but consistent.
...
def _build_context_from_retrieval(res: Dict, max_chars: int = MAX_CONTEXT_CHARS):
    ...
    for i in range(len(ids)):
        ...
        src = {"rank": i + 1, "chunk_id": ids[i], "doc_name": metas[i].get("doc_name"),
               "page_start": metas[i].get("page_start"), "page_end": metas[i].get("page_end"), "distance": dists[i]}
        sources.append(src)
        evidence.append(src.copy());
        evidence[-1]["text"] = snippet
        block = f"[SOURCE {i + 1}] doc={src['doc_name']} pages={src['page_start']}-{src['page_end']} dist={dists[i]:.3f}\n{snippet}\n"
...
def rag_answer(question: str, api_key: Optional[str] = None, top_k: int = RAG_TOP_K) -> Dict:
    if not question.strip(): raise ValueError("Empty question")
    ...
    low_similarity = sources[0].get("distance", 0) > RAG_DISTANCE_THRESHOLD
    instruction = build_answer_instruction(intent)
    client = _openai_client(api_key)

    user_msg = f"{_build_declarations_block(intent)}Question: {question}\n\nContext: {context_text}\n\nRules: Use ONLY context. Follow structure:\n{instruction}"

    resp = client.chat.completions.create(
        model=CHAT_MODEL,
        messages=[{"role": "system", "content": "You are PolicyPal. Use provided context only."},
                  {"role": "user", "content": user_msg}],
        temperature=0.2,
    )

    answer = _enforce_sources_used_line(resp.choices[0].message.content.strip(), sources[:top_k])
